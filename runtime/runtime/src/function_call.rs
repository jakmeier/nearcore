...
pub(crate) fn execute_function_call(
    ...
    // function-call outcome instead of panicking: a consensus failure on one
    // chunk is preferable to crashing all nodes if a VM bug surfaces
    // post-deploy. User-code errors are reported via `outcome.aborted` and
